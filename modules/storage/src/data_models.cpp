...
NodeModel NodeModel::from_db_result(const Database::Result& result) {
    NodeModel model;
    
    // Extract node_id as blob
    auto node_id_blob = result.get_blob("node_id");
    if (node_id_blob.size() == types::NodeID::SIZE) {
        model.node_id = types::NodeID(node_id_blob.data());
    }
...
InfoHashModel InfoHashModel::from_db_result(const Database::Result& result) {
    InfoHashModel model;
    
    // Extract info_hash as blob
    auto info_hash_blob = result.get_blob("info_hash");
    if (info_hash_blob.size() == types::InfoHash::SIZE) {
        model.info_hash = types::InfoHash(info_hash_blob.data());
    }
...
MetadataModel MetadataModel::from_db_result(const Database::Result& result) {
    MetadataModel model;
...
FileModel FileModel::from_db_result(const Database::Result& result) {
    FileModel model;
...
TrackerModel TrackerModel::from_db_result(const Database::Result& result) {
    TrackerModel model;
...
PeerModel PeerModel::from_db_result(const Database::Result& result) {
    PeerModel model;
    ...
    if (!result.is_null("peer_id")) {
        ...
        if (peer_id_blob.size() == types::NodeID::SIZE) {
            model.peer_id = types::NodeID(peer_id_blob.data());
        }
    }
