...
bool Endpoint::is_valid() const { return !address_.empty() && port_ > 0; }
...
Endpoint Endpoint::resolve(const std::string &host, uint16_t port,
                           AddressType type) {
  ...
  return Endpoint(std::string(ip_str), port);
}

std::future<Endpoint> Endpoint::resolve_async(const std::string &host,
                                              uint16_t port, AddressType type) {
  return std::async(std::launch::async,
                    [host, port, type]() { return resolve(host, port, type); });
}
