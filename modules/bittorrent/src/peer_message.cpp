...
std::shared_ptr<PeerMessage> PeerMessageFactory::create_from_data(const std::vector<uint8_t>& data) {
    ...
    if (type == PeerMessageType::CHOKE) {
        ...
        uint32_t piece_index = (data[1] << 24) | (data[2] << 16) | (data[3] << 8) | data[4];
        ...
        if (data.size() < 13) { // 1 byte type + 4 bytes index + 4 bytes begin + 4 bytes length
            return nullptr;
        }
        uint32_t index = (data[1] << 24) | (data[2] << 16) | (data[3] << 8) | data[4];
        uint32_t begin = (data[5] << 24) | (data[6] << 16) | (data[7] << 8) | data[8];
        uint32_t length = (data[9] << 24) | (data[10] << 16) | (data[11] << 8) | data[12];
        return create_request(index, begin, length);
    } else if (type == PeerMessageType::PIECE) {
        if (data.size() < 9) { // 1 byte type + 4 bytes index + 4 bytes begin + at least 0 bytes block
            return nullptr;
        }
        uint32_t index = (data[1] << 24) | (data[2] << 16) | (data[3] << 8) | data[4];
        uint32_t begin = (data[5] << 24) | (data[6] << 16) | (data[7] << 8) | data[8];
...
ChokeMessage::ChokeMessage()
    : PeerMessage(PeerMessageType::CHOKE) {
}
...
std::string ChokeMessage::to_string() const {
    return "ChokeMessage[]";
}
...
UnchokeMessage::UnchokeMessage()
    : PeerMessage(PeerMessageType::UNCHOKE) {
}
...
std::string UnchokeMessage::to_string() const {
...
HaveMessage::HaveMessage(uint32_t piece_index)
    : PeerMessage(PeerMessageType::HAVE),
      piece_index_(piece_index) {
}

uint32_t HaveMessage::piece_index() const {
    return piece_index_;
}
...
BitfieldMessage::BitfieldMessage(const std::vector<uint8_t>& bitfield)
    : PeerMessage(PeerMessageType::BITFIELD),
      bitfield_(bitfield) {
}
...
std::vector<uint8_t> BitfieldMessage::serialize() const {
    ...
    // Set length (4 bytes, big-endian)
    uint32_t length = 1 + static_cast<uint32_t>(bitfield_.size()); // 1 byte ID + bitfield
...
RequestMessage::RequestMessage(uint32_t index, uint32_t begin, uint32_t length)
    : PeerMessage(PeerMessageType::REQUEST),
      index_(index),
      begin_(begin),
      length_(length) {
}

uint32_t RequestMessage::index() const {
    return index_;
}

uint32_t RequestMessage::begin() const {
    return begin_;
}

uint32_t RequestMessage::length() const {
    return length_;
}
...
std::vector<uint8_t> RequestMessage::serialize() const {
    ...
    // Set length (4 bytes, big-endian)
    result[3] = 13; // Length is 13 bytes (1 byte ID + 4 bytes index + 4 bytes begin + 4 bytes length)
...
std::string RequestMessage::to_string() const {
    std::ostringstream oss;
    oss << "RequestMessage[index=" << index_ << ", begin=" << begin_ << ", length=" << length_ << "]";
    return oss.str();
}

// PieceMessage implementation

PieceMessage::PieceMessage(uint32_t index, uint32_t begin, const std::vector<uint8_t>& block)
    : PeerMessage(PeerMessageType::PIECE),
      index_(index),
      begin_(begin),
      block_(block) {
}

uint32_t PieceMessage::index() const {
    return index_;
}

uint32_t PieceMessage::begin() const {
    return begin_;
}

const std::vector<uint8_t>& PieceMessage::block() const {
    return block_;
}
...
std::vector<uint8_t> PieceMessage::serialize() const {
    ...
    // Set length (4 bytes, big-endian)
    uint32_t length = 9 + static_cast<uint32_t>(block_.size()); // 1 byte ID + 4 bytes index + 4 bytes begin + block
    result[0] = static_cast<uint8_t>((length >> 24) & 0xFF);
    result[1] = static_cast<uint8_t>((length >> 16) & 0xFF);
    result[2] = static_cast<uint8_t>((length >> 8) & 0xFF);
    result[3] = static_cast<uint8_t>(length & 0xFF);
...
std::string PieceMessage::to_string() const {
    std::ostringstream oss;
    oss << "PieceMessage[index=" << index_ << ", begin=" << begin_ << ", block_size=" << block_.size() << "]";
    return oss.str();
}
...
CancelMessage::CancelMessage(uint32_t index, uint32_t begin, uint32_t length)
    : PeerMessage(PeerMessageType::CANCEL),
      index_(index),
      begin_(begin),
      length_(length) {
}

uint32_t CancelMessage::index() const {
    return index_;
}

uint32_t CancelMessage::begin() const {
    return begin_;
}

uint32_t CancelMessage::length() const {
    return length_;
}
...
std::string CancelMessage::to_string() const {
    std::ostringstream oss;
    oss << "CancelMessage[index=" << index_ << ", begin=" << begin_ << ", length=" << length_ << "]";
    return oss.str();
}
...
PortMessage::PortMessage(uint16_t port)
    : PeerMessage(PeerMessageType::PORT),
      port_(port) {
}

uint16_t PortMessage::port() const {
    return port_;
}
...
std::shared_ptr<NotInterestedMessage> PeerMessageFactory::create_not_interested() {
    return std::make_shared<NotInterestedMessage>();
}

std::shared_ptr<HaveMessage> PeerMessageFactory::create_have(uint32_t piece_index) {
    return std::make_shared<HaveMessage>(piece_index);
}

std::shared_ptr<BitfieldMessage> PeerMessageFactory::create_bitfield(const std::vector<uint8_t>& bitfield) {
    return std::make_shared<BitfieldMessage>(bitfield);
}

std::shared_ptr<RequestMessage> PeerMessageFactory::create_request(uint32_t index, uint32_t begin, uint32_t length) {
    return std::make_shared<RequestMessage>(index, begin, length);
}

std::shared_ptr<PieceMessage> PeerMessageFactory::create_piece(uint32_t index, uint32_t begin, const std::vector<uint8_t>& block) {
    return std::make_shared<PieceMessage>(index, begin, block);
}

std::shared_ptr<CancelMessage> PeerMessageFactory::create_cancel(uint32_t index, uint32_t begin, uint32_t length) {
