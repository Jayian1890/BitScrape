...
InfoHash InfoHash::from_bencode(const std::vector<uint8_t>& data) {
    // This is a simplified implementation that just calculates SHA1 of the data
    // In a real implementation, we would parse the bencode data and extract the info dictionary
...
std::future<InfoHash> InfoHash::from_bencode_async(const std::vector<uint8_t>& data) {
    return std::async(std::launch::async, [data]() {
        return from_bencode(data);
    });
}
