...
public:
  ...
  bool initialize() {
    ...
    try {
      ...
      std::cerr << "Failed to initialize storage manager: " << e.what()
                << std::endl;
      return false;
    }
  }
  ...
  bool update_node_responsiveness(const types::NodeID &node_id,
                                  bool is_responsive) {
    std::lock_guard<std::mutex> lock(mutex_);
    ...
    try {
      ...
      std::cerr << "Failed to update node responsiveness: " << e.what()
                << std::endl;
      return false;
    }
  }
  ...
  std::future<bool>
  update_node_responsiveness_async(const types::NodeID &node_id,
                                   bool is_responsive) {
  ...
  bool increment_node_ping_count(const types::NodeID &node_id) {
    ...
    try {
      ...
      return database_->execute_update(
          "UPDATE nodes SET last_seen = ?, ping_count = ping_count + 1 WHERE "
          "node_id = ?",
          {time_point_to_string(now), node_id.to_hex()});
    } catch (const std::exception &e) {
      std::cerr << "Failed to increment node ping count: " << e.what()
                << std::endl;
      return false;
    }
  }
  ...
  std::future<bool>
  increment_node_ping_count_async(const types::NodeID &node_id) {
  ...
  bool increment_node_query_count(const types::NodeID &node_id) {
    ...
    try {
      ...
      return database_->execute_update(
          "UPDATE nodes SET last_seen = ?, query_count = query_count + 1 WHERE "
          "node_id = ?",
          {time_point_to_string(now), node_id.to_hex()});
    } catch (const std::exception &e) {
      std::cerr << "Failed to increment node query count: " << e.what()
                << std::endl;
      return false;
    }
  }
  ...
  std::future<bool>
  increment_node_query_count_async(const types::NodeID &node_id) {
    return std::async(std::launch::async, [this, node_id]() {
  ...
  bool increment_node_response_count(const types::NodeID &node_id) {
    ...
    try {
      ...
      return database_->execute_update(
          "UPDATE nodes SET last_seen = ?, response_count = response_count + 1 "
          "WHERE node_id = ?",
          {time_point_to_string(now), node_id.to_hex()});
    } catch (const std::exception &e) {
      std::cerr << "Failed to increment node response count: " << e.what()
                << std::endl;
      return false;
    }
  }
  ...
  std::future<bool>
  increment_node_response_count_async(const types::NodeID &node_id) {
    return std::async(std::launch::async, [this, node_id]() {
    ...
    try {
      ...
      if (existing_infohash) {
        ...
        return database_->execute_update(
            ...
            {time_point_to_string(now), info_hash.to_hex()});
      } else {
        // Insert new infohash
        auto now = std::chrono::system_clock::now();
        ...
        return database_->execute_update(
            ...
            "(?, ?, ?)",
            {info_hash.to_hex(), time_point_to_string(now),
             time_point_to_string(now)});
      }
    } catch (const std::exception &e) {
  ...
  bool increment_infohash_announce_count(const types::InfoHash &info_hash) {
    ...
    try {
      ...
      return database_->execute_update(
          "UPDATE infohashes SET last_seen = ?, announce_count = "
          "announce_count + 1 WHERE info_hash = ?",
          {time_point_to_string(now), info_hash.to_hex()});
    } catch (const std::exception &e) {
      std::cerr << "Failed to increment infohash announce count: " << e.what()
                << std::endl;
      return false;
    }
  }
  ...
  std::future<bool>
  increment_infohash_announce_count_async(const types::InfoHash &info_hash) {
    return std::async(std::launch::async, [this, info_hash]() {
  ...
  bool increment_infohash_peer_count(const types::InfoHash &info_hash) {
    ...
    try {
      ...
      return database_->execute_update(
          "UPDATE infohashes SET last_seen = ?, peer_count = peer_count + 1 "
          "WHERE info_hash = ?",
          {time_point_to_string(now), info_hash.to_hex()});
    } catch (const std::exception &e) {
      std::cerr << "Failed to increment infohash peer count: " << e.what()
                << std::endl;
      return false;
    }
  }
  ...
  std::future<bool>
  increment_infohash_peer_count_async(const types::InfoHash &info_hash) {
    return std::async(std::launch::async, [this, info_hash]() {
      return increment_infohash_peer_count(info_hash);
    });
  }

  bool store_metadata(const types::InfoHash &info_hash,
                      const types::MetadataInfo &metadata) {
    std::lock_guard<std::mutex> lock(mutex_);
    ...
    try {
      ...
      if (existing_metadata) {
        ...
        return database_->execute_update(
            "UPDATE metadata SET download_time = ?, name = ?, total_size = ?, "
            "piece_count = ?, file_count = ? WHERE info_hash = ?",
            {time_point_to_string(now),
             metadata.name().empty() ? "Unknown" : metadata.name(),
             std::to_string(metadata.total_size()),
             std::to_string(metadata.pieces().size()),
             std::to_string(metadata.files().size()), info_hash.to_hex()});
      } else {
        // Insert new metadata
        auto now = std::chrono::system_clock::now();

        // Extract metadata fields from the MetadataInfo object
        std::string name =
            metadata.name().empty() ? "Unknown" : metadata.name();
        uint64_t total_size = metadata.total_size();
        size_t piece_count = metadata.pieces().size();
        size_t file_count = metadata.files().size();

        // Convert raw_data to a string for storage (hex or base64 encoding
        // could be used)
        std::string raw_meta_str;
        const auto &raw = metadata.raw_data();
        if (!raw.empty()) {
          raw_meta_str.assign(reinterpret_cast<const char *>(raw.data()),
                              raw.size());
        }

        return database_->execute_update(
            "INSERT INTO metadata (info_hash, download_time, name, total_size, "
            "piece_count, file_count, raw_metadata) "
            "VALUES (?, ?, ?, ?, ?, ?, ?)",
            {info_hash.to_hex(), time_point_to_string(now), name,
             std::to_string(total_size), std::to_string(piece_count),
             std::to_string(file_count), raw_meta_str});
      }
    } catch (const std::exception &e) {
  ...
  std::future<bool> store_metadata_async(const types::InfoHash &info_hash,
                                         const types::MetadataInfo &metadata) {
  ...
  bool store_torrent(const types::InfoHash &info_hash,
                     const types::TorrentInfo &torrent_info) {
    std::lock_guard<std::mutex> lock(mutex_);
    ...
    try {
      ...
      if (!torrent_info.announce().empty()) {
        if (!database_->execute_update(
                ...
                {info_hash.to_hex(), torrent_info.announce()})) {
          database_->rollback_transaction();
          return false;
        }
      }
  ...
  std::future<bool>
  store_torrent_async(const types::InfoHash &info_hash,
                      const types::TorrentInfo &torrent_info) {
  ...
  bool store_peer(const types::InfoHash &info_hash,
                  const types::Endpoint &endpoint,
                  const std::optional<types::NodeID> &peer_id,
                  bool supports_dht, bool supports_extension_protocol,
    ...
    try {
      ...
      auto result = database_->execute(
          ...
          {info_hash.to_hex(), endpoint.address(),
           std::to_string(endpoint.port())});
      ...
      if (result.next()) {
        ...
        return database_->execute_update(
            ...
            "supports_dht = ?, supports_extension_protocol = ?, "
            "supports_fast_protocol = ? "
            "WHERE info_hash = ? AND ip = ? AND port = ?",
            {time_point_to_string(now), peer_id_hex, supports_dht ? "1" : "0",
             supports_extension_protocol ? "1" : "0",
             supports_fast_protocol ? "1" : "0", info_hash.to_hex(),
        ...
        return database_->execute_update(
            "INSERT INTO peers (info_hash, ip, port, peer_id, first_seen, "
            "last_seen, "
            "supports_dht, supports_extension_protocol, "
            "supports_fast_protocol) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
            {info_hash.to_hex(), endpoint.address(),
             std::to_string(endpoint.port()), peer_id_hex,
             time_point_to_string(now), time_point_to_string(now),
             supports_dht ? "1" : "0", supports_extension_protocol ? "1" : "0",
  ...
  std::future<bool> store_peer_async(
      const types::InfoHash &info_hash, const types::Endpoint &endpoint,
      const std::optional<types::NodeID> &peer_id, bool supports_dht,
      bool supports_extension_protocol, bool supports_fast_protocol) {
    return std::async(std::launch::async, [this, info_hash, endpoint, peer_id,
                                           supports_dht,
                                           supports_extension_protocol,
                                           supports_fast_protocol]() {
      return store_peer(info_hash, endpoint, peer_id, supports_dht,
                        supports_extension_protocol, supports_fast_protocol);
    });
  }
    ...
    try {
      ...
      if (result.next()) {
        ...
        return database_->execute_update(
            ...
            {time_point_to_string(now), info_hash.to_hex(), url});
      } else {
        // Insert new tracker
        auto now = std::chrono::system_clock::now();

        return database_->execute_update(
            "INSERT INTO trackers (info_hash, url, first_seen, last_seen) "
            "VALUES (?, ?, ?, ?)",
            {info_hash.to_hex(), url, time_point_to_string(now),
             time_point_to_string(now)});
      }
    } catch (const std::exception &e) {
  ...
  std::future<bool> store_tracker_async(const types::InfoHash &info_hash,
  ...
  bool increment_tracker_announce_count(const types::InfoHash &info_hash,
                                        const std::string &url) {
    std::lock_guard<std::mutex> lock(mutex_);
    ...
    try {
      ...
      return database_->execute_update(
          "UPDATE trackers SET last_seen = ?, announce_count = announce_count "
          "+ 1 WHERE info_hash = ? AND url = ?",
          {time_point_to_string(now), info_hash.to_hex(), url});
    } catch (const std::exception &e) {
      std::cerr << "Failed to increment tracker announce count: " << e.what()
                << std::endl;
      return false;
    }
  }
  ...
  std::future<bool>
  increment_tracker_announce_count_async(const types::InfoHash &info_hash,
                                         const std::string &url) {
    return std::async(std::launch::async, [this, info_hash, url]() {
  ...
  bool increment_tracker_scrape_count(const types::InfoHash &info_hash,
                                      const std::string &url) {
    std::lock_guard<std::mutex> lock(mutex_);
    ...
    try {
      ...
      return database_->execute_update(
          "UPDATE trackers SET last_seen = ?, scrape_count = scrape_count + 1 "
          "WHERE info_hash = ? AND url = ?",
          {time_point_to_string(now), info_hash.to_hex(), url});
    } catch (const std::exception &e) {
      std::cerr << "Failed to increment tracker scrape count: " << e.what()
                << std::endl;
      return false;
    }
  }
  ...
  std::future<bool>
  increment_tracker_scrape_count_async(const types::InfoHash &info_hash,
                                       const std::string &url) {
    return std::async(std::launch::async, [this, info_hash, url]() {
  ...
  std::unordered_map<std::string, std::string> get_statistics() const {
    ...
    try {
      ...
      if (result.next()) {
        stats["storage.infohash_count"] =
            std::to_string(result.get_int("count"));
      }

      // Get metadata count
      result = database_->execute("SELECT COUNT(*) AS count FROM metadata");
      if (result.next()) {
        stats["storage.metadata_count"] =
            std::to_string(result.get_int("count"));
      }
  ...
  std::future<std::unordered_map<std::string, std::string>>
  get_statistics_async() const {
    return std::async(std::launch::async,
                      [this]() { return get_statistics(); });
  }
...
bool StorageManager::initialize() { return impl_->initialize(); }

std::future<bool> StorageManager::initialize_async() {
  return impl_->initialize_async();
}

bool StorageManager::close() { return impl_->close(); }

std::future<bool> StorageManager::close_async() { return impl_->close_async(); }
...
StorageManager::get_statistics() const {
  return impl_->get_statistics();
}

std::future<std::unordered_map<std::string, std::string>>
StorageManager::get_statistics_async() const {
  return impl_->get_statistics_async();
}

bool StorageManager::update_node_responsiveness(const types::NodeID &node_id,
                                                bool is_responsive) {
  return impl_->update_node_responsiveness(node_id, is_responsive);
}

std::future<bool>
StorageManager::update_node_responsiveness_async(const types::NodeID &node_id,
                                                 bool is_responsive) {
...
std::future<bool>
StorageManager::increment_node_ping_count_async(const types::NodeID &node_id) {
...
std::future<bool>
StorageManager::increment_node_query_count_async(const types::NodeID &node_id) {
  return impl_->increment_node_query_count_async(node_id);
}

bool StorageManager::increment_node_response_count(
    const types::NodeID &node_id) {
  return impl_->increment_node_response_count(node_id);
}

std::future<bool> StorageManager::increment_node_response_count_async(
    const types::NodeID &node_id) {
...
std::future<bool>
StorageManager::store_infohash_async(const types::InfoHash &info_hash) {
  return impl_->store_infohash_async(info_hash);
}

bool StorageManager::increment_infohash_announce_count(
    const types::InfoHash &info_hash) {
  return impl_->increment_infohash_announce_count(info_hash);
}

std::future<bool> StorageManager::increment_infohash_announce_count_async(
    const types::InfoHash &info_hash) {
  return impl_->increment_infohash_announce_count_async(info_hash);
}

bool StorageManager::increment_infohash_peer_count(
    const types::InfoHash &info_hash) {
  return impl_->increment_infohash_peer_count(info_hash);
}

std::future<bool> StorageManager::increment_infohash_peer_count_async(
    const types::InfoHash &info_hash) {
  return impl_->increment_infohash_peer_count_async(info_hash);
}

bool StorageManager::store_metadata(const types::InfoHash &info_hash,
                                    const types::MetadataInfo &metadata) {
  return impl_->store_metadata(info_hash, metadata);
}

std::future<bool>
StorageManager::store_metadata_async(const types::InfoHash &info_hash,
                                     const types::MetadataInfo &metadata) {
  return impl_->store_metadata_async(info_hash, metadata);
}

bool StorageManager::store_torrent(const types::InfoHash &info_hash,
                                   const types::TorrentInfo &torrent_info) {
  return impl_->store_torrent(info_hash, torrent_info);
}

std::future<bool>
StorageManager::store_torrent_async(const types::InfoHash &info_hash,
                                    const types::TorrentInfo &torrent_info) {
  return impl_->store_torrent_async(info_hash, torrent_info);
}

bool StorageManager::store_peer(const types::InfoHash &info_hash,
                                const types::Endpoint &endpoint,
                                const std::optional<types::NodeID> &peer_id,
                                bool supports_dht,
                                bool supports_extension_protocol,
                                bool supports_fast_protocol) {
  return impl_->store_peer(info_hash, endpoint, peer_id, supports_dht,
                           supports_extension_protocol, supports_fast_protocol);
}

std::future<bool> StorageManager::store_peer_async(
    const types::InfoHash &info_hash, const types::Endpoint &endpoint,
    const std::optional<types::NodeID> &peer_id, bool supports_dht,
    bool supports_extension_protocol, bool supports_fast_protocol) {
  return impl_->store_peer_async(info_hash, endpoint, peer_id, supports_dht,
                                 supports_extension_protocol,
                                 supports_fast_protocol);
}

bool StorageManager::store_tracker(const types::InfoHash &info_hash,
                                   const std::string &url) {
  return impl_->store_tracker(info_hash, url);
}

std::future<bool>
StorageManager::store_tracker_async(const types::InfoHash &info_hash,
                                    const std::string &url) {
  return impl_->store_tracker_async(info_hash, url);
}

bool StorageManager::increment_tracker_announce_count(
    const types::InfoHash &info_hash, const std::string &url) {
  return impl_->increment_tracker_announce_count(info_hash, url);
}

std::future<bool> StorageManager::increment_tracker_announce_count_async(
    const types::InfoHash &info_hash, const std::string &url) {
  return impl_->increment_tracker_announce_count_async(info_hash, url);
}

bool StorageManager::increment_tracker_scrape_count(
    const types::InfoHash &info_hash, const std::string &url) {
  return impl_->increment_tracker_scrape_count(info_hash, url);
}

std::future<bool> StorageManager::increment_tracker_scrape_count_async(
    const types::InfoHash &info_hash, const std::string &url) {
  return impl_->increment_tracker_scrape_count_async(info_hash, url);
}

std::shared_ptr<StorageManager>
create_storage_manager(const std::string &db_path, bool persistent) {
