...
std::string HandshakeMessage::to_string() const {
    ...
    for (size_t i = 0; i < info_hash_.size(); ++i) {
        oss << std::hex << std::setw(2) << std::setfill('0') << static_cast<int>(info_hash_[i]);
    }
    
    oss << ", peer_id=";
    
    // Format peer_id as hex
    for (size_t i = 0; i < peer_id_.size(); ++i) {
        oss << std::hex << std::setw(2) << std::setfill('0') << static_cast<int>(peer_id_[i]);
    }
