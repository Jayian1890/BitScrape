...
public:
     ...
     */
    void set_metadata_received_callback(std::function<void(const types::MetadataInfo&)> callback);
...
private:
     ...
     */
    void handle_extended_handshake(const network::Address& address, const bencode::BencodeValue& message);
     ...
     */
    void handle_metadata_message(const network::Address& address, const bencode::BencodeValue& message);
    ...
    std::unordered_map<std::string, int> peer_metadata_size_; ///< Metadata size reported by peers
    std::unordered_map<int, std::vector<uint8_t>> metadata_pieces_; ///< Received metadata pieces
    mutable std::mutex pieces_mutex_;         ///< Mutex for pieces map

    std::function<void(const types::MetadataInfo&)> metadata_received_callback_; ///< Callback for metadata received
