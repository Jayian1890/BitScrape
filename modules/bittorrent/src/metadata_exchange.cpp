...
void MetadataExchange::initialize() {
  ...
  protocol_.register_message_handler(
      PeerMessageType::EXTENDED,
      [this](const network::Address &address, const PeerMessage &message) {
        // Cast to ExtendedMessage
        const auto *extended_message =
            dynamic_cast<const ExtendedMessage *>(&message);
        if (!extended_message) {
          return;
        }
...
std::future<bool>
MetadataExchange::request_metadata_async(const network::Address &address) {
    ...
    if (total_size > 0) {
      ...
      std::vector<uint8_t> piece_data;
      {
        ...
        if (metadata_) {
          ...
          piece_data.assign(data.begin() + offset,
                            data.begin() + offset + piece_size);
        }
      }
...
bool MetadataExchange::send_extended_handshake(
  ...
  m_dict["ut_metadata"] = bencode::BencodeValue(
      static_cast<int64_t>(1)); // We use ID 1 for ut_metadata
  ...
  // Add metadata_size if we have it
  {
    std::lock_guard<std::mutex> lock(metadata_mutex_);
    if (metadata_) {
      handshake_dict["metadata_size"] = bencode::BencodeValue(
          static_cast<int64_t>(metadata_->raw_data().size()));
    }
  }
...
bool MetadataExchange::send_metadata_request(const network::Address &address,
  ...
  request_dict["msg_type"] =
      bencode::BencodeValue(static_cast<int64_t>(0)); // 0 = request
...
bool MetadataExchange::send_metadata_data(
    const network::Address &address, int piece_index, int total_size,
  ...
  message_dict["msg_type"] =
      bencode::BencodeValue(static_cast<int64_t>(1)); // 1 = data
  message_dict["piece"] =
      bencode::BencodeValue(static_cast<int64_t>(piece_index));
  message_dict["total_size"] =
      bencode::BencodeValue(static_cast<int64_t>(total_size));
  ...
  encoded_message.insert(encoded_message.end(), piece_data.begin(),
                         piece_data.end());

  // Create the extended message with the encoded message and piece data
  // We can't use the normal ExtendedMessage constructor because we need to
  // append the piece data to the bencoded dictionary, not include it in the
  // dictionary

  // First, create a message with just the bencoded dictionary
  auto extended_message =
      std::make_shared<ExtendedMessage>(ut_metadata_id_, message);
  ...
  // Add message data
  final_message.insert(final_message.end(), message_data.begin(),
                       message_data.end());
...
bool MetadataExchange::send_metadata_reject(const network::Address &address,
                                            int piece) {
  // Check if ut_metadata is supported
  if (ut_metadata_id_ == 0) {
    return false;
  ...
  reject_dict["msg_type"] =
      bencode::BencodeValue(static_cast<int64_t>(2)); // 2 = reject
  ...
  // Create the extended message
  auto extended_message =
      std::make_shared<ExtendedMessage>(ut_metadata_id_, reject);
