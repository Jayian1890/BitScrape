...
TokenManager::TokenManager(lock::LockManager &lock_manager)
    : last_rotation_(std::chrono::system_clock::now()),
      ...
      resource_id_(lock_manager.register_resource(
          get_resource_name(), lock::LockManager::LockPriority::NORMAL)) {
...
types::DHTToken TokenManager::generate_token(const types::Endpoint &endpoint) {
  auto guard = lock_manager_.get_lock_guard(
      resource_id_, lock::LockManager::LockType::EXCLUSIVE);

  // Check if we need to rotate the secret
  auto now = std::chrono::system_clock::now();
  if (std::chrono::duration_cast<std::chrono::seconds>(now - last_rotation_)
          .count() >= TOKEN_ROTATION_INTERVAL) {
    rotate_secret();
    last_rotation_ = now;
  }
...
std::future<types::DHTToken>
TokenManager::generate_token_async(const types::Endpoint &endpoint) {
  return std::async(std::launch::async, [this, endpoint]() {
    return this->generate_token(endpoint);
  });
}

bool TokenManager::verify_token(const types::DHTToken &token,
                                const types::Endpoint &endpoint) {
  auto guard = lock_manager_.get_lock_guard(
      resource_id_, lock::LockManager::LockType::SHARED);
...
std::future<bool>
TokenManager::verify_token_async(const types::DHTToken &token,
                                 const types::Endpoint &endpoint) {
...
types::DHTToken
TokenManager::generate_token_with_secret(const types::Endpoint &endpoint,
