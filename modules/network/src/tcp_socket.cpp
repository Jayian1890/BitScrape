...
bool TCPSocket::connect(const Address &address) {
  ...
  if (::connect(socket_->descriptor(), reinterpret_cast<sockaddr *>(&addr),
                sizeof(addr)) == -1) {
    return false;
  }
...
std::future<bool> TCPSocket::connect_async(const Address &address) {
  return std::async(std::launch::async,
                    [this, address]() { return connect(address); });
}
...
bool TCPSocket::is_valid() const { return socket_->is_valid(); }
...
int TCPSocket::send(const uint8_t *data, size_t size) {
  ...
  return ::send(socket_->descriptor(), reinterpret_cast<const char *>(data),
                size, 0);
}

std::future<int> TCPSocket::send_async(const uint8_t *data, size_t size) {
  return std::async(std::launch::async,
                    [this, data, size]() { return send(data, size); });
}
...
std::future<int> TCPSocket::send_async(const Buffer &buffer) {
  return std::async(std::launch::async,
                    [this, buffer]() { return send(buffer); });
}
...
int TCPSocket::receive(uint8_t *data, size_t size) {
  ...
  return recv(socket_->descriptor(), reinterpret_cast<char *>(data), size, 0);
}

std::future<int> TCPSocket::receive_async(uint8_t *data, size_t size) {
  return std::async(std::launch::async,
                    [this, data, size]() { return receive(data, size); });
}
...
std::future<int> TCPSocket::receive_async(Buffer &buffer) {
  return std::async(std::launch::async,
                    [this, &buffer]() { return receive(buffer); });
}
...
Address TCPSocket::get_local_address() const {
  ...
  if (getsockname(socket_->descriptor(), reinterpret_cast<sockaddr *>(&addr),
                  &addr_len) == -1) {
    return Address(0);
  }
...
Address TCPSocket::get_remote_address() const {
  ...
  if (getpeername(socket_->descriptor(), reinterpret_cast<sockaddr *>(&addr),
                  &addr_len) == -1) {
    return Address(0);
  }
