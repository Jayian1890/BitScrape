...
std::shared_ptr<DHTGetPeersMessage> DHTMessageFactory::create_get_peers(const std::string& transaction_id,
                                                                      const types::NodeID& node_id,
                                                                      const types::InfoHash& info_hash) {
    return std::make_shared<DHTGetPeersMessage>(transaction_id, node_id, info_hash);
}

std::shared_ptr<DHTGetPeersMessage> DHTMessageFactory::create_get_peers_response(const std::string& transaction_id,
                                                                               const types::NodeID& node_id,
                                                                               const types::DHTToken& token,
                                                                               const std::vector<types::DHTNode>& nodes,
                                                                               const std::vector<types::Endpoint>& values) {
    return std::make_shared<DHTGetPeersMessage>(transaction_id, node_id, token, nodes, values);
}

std::shared_ptr<DHTFindNodeMessage> DHTMessageFactory::create_find_node(const std::string& transaction_id,
                                                                      const types::NodeID& node_id,
                                                                      const types::NodeID& target_id) {
    return std::make_shared<DHTFindNodeMessage>(transaction_id, node_id, target_id);
}

std::shared_ptr<DHTFindNodeMessage> DHTMessageFactory::create_find_node_response(const std::string& transaction_id,
                                                                               const types::NodeID& node_id,
                                                                               const std::vector<types::DHTNode>& nodes) {
    return std::make_shared<DHTFindNodeMessage>(transaction_id, node_id, nodes);
}

std::shared_ptr<DHTAnnouncePeerMessage> DHTMessageFactory::create_announce_peer(const std::string& transaction_id,
                                                                              const types::NodeID& node_id,
                                                                              const types::InfoHash& info_hash,
                                                                              uint16_t port,
                                                                              const types::DHTToken& token,
                                                                              bool implied_port) {
    return std::make_shared<DHTAnnouncePeerMessage>(transaction_id, node_id, info_hash, port, token, implied_port);
}

std::shared_ptr<DHTAnnouncePeerMessage> DHTMessageFactory::create_announce_peer_response(const std::string& transaction_id,
                                                                                       const types::NodeID& node_id) {
...
std::shared_ptr<DHTGetPeersMessage> DHTMessageFactory::parse_get_peers(const bencode::BencodeValue& value,
                                                                      const std::string& transaction_id,
                                                                      bool is_response) {
    // Get the arguments or response dictionary
    std::string dict_key = is_response ? "r" : "a";
    ...
    if (!is_response) {
        ...
        try {
            info_hash = types::InfoHash(std::vector<uint8_t>(info_hash_str.begin(), info_hash_str.end()));
        } catch (const std::exception& e) {
        ...
        types::DHTToken token(std::vector<uint8_t>(token_str.begin(), token_str.end()));
        ...
        if (nodes_value && nodes_value->is_string()) {
            ...
            // Parse the nodes string (26 bytes per node: 20 bytes ID + 6 bytes endpoint)
            // This is a simplified implementation - in a real implementation,
            // we would need to parse the compact node info format
            // For now, just create a dummy node
            if (!nodes_str.empty()) {
                nodes.push_back(types::DHTNode(types::NodeID(), types::Endpoint(std::string("0.0.0.0"), 0)));
            }
        }
        ...
        if (values_value && values_value->is_list()) {
            ...
            for (const auto& value : values_list) {
                if (value.is_string()) {
                    ...
                    // This is a simplified implementation - in a real implementation,
                    // we would need to parse the compact peer info format
...
std::shared_ptr<DHTFindNodeMessage> DHTMessageFactory::parse_find_node(const bencode::BencodeValue& value,
                                                                      const std::string& transaction_id,
                                                                      bool is_response) {
    // Get the arguments or response dictionary
    std::string dict_key = is_response ? "r" : "a";
    ...
    if (!is_response) {
        ...
        try {
            target_id = types::NodeID(std::vector<uint8_t>(target_str.begin(), target_str.end()));
        } catch (const std::exception& e) {
            // Failed to create the NodeID
            return nullptr;
        }
...
std::shared_ptr<DHTAnnouncePeerMessage> DHTMessageFactory::parse_announce_peer(const bencode::BencodeValue& value,
                                                                             const std::string& transaction_id,
                                                                             bool is_response) {
    // Get the arguments or response dictionary
    std::string dict_key = is_response ? "r" : "a";
    ...
    if (!is_response) {
        ...
        const bencode::BencodeValue* implied_port_value = dict_value->get("implied_port");
...
std::shared_ptr<DHTMessage> DHTMessageFactory::parse_error(const bencode::BencodeValue& value,
                                                          const std::string& transaction_id) {
    // Get the error array
