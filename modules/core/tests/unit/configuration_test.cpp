...
TEST_F(ConfigurationTest, SetGetEndpointListTest) {
    ...
    std::vector<types::Endpoint> test_list = {
        types::Endpoint("192.168.1.1", 6881),
        types::Endpoint("example.com", 6882),
        types::Endpoint("10.0.0.1", 6883)
    };
    ...
    EXPECT_EQ(test_list.size(), result.size());
    for (size_t i = 0; i < test_list.size(); ++i) {
        EXPECT_EQ(test_list[i].get_address(), result[i].get_address());
        EXPECT_EQ(test_list[i].get_port(), result[i].get_port());
    }
    ...
    EXPECT_EQ(test_list.size(), loaded_result.size());
    for (size_t i = 0; i < test_list.size(); ++i) {
        EXPECT_EQ(test_list[i].get_address(), loaded_result[i].get_address());
        EXPECT_EQ(test_list[i].get_port(), loaded_result[i].get_port());
    }
}
