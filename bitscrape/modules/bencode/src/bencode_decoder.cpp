...
public:
     ...
     */
    std::future<BencodeValue> decode_async(const std::vector<uint8_t>& data) override {
     ...
     * @return Future containing the decoded BencodeValue
     */
    std::future<BencodeValue> decode_async(const uint8_t* data, size_t size) override {
...
private:
    ...
    BencodeValue decode_string(const uint8_t* data, size_t size, size_t& pos) {
        ...
        // Parse the length
        std::string length_str(reinterpret_cast<const char*>(data + length_start), pos - length_start);
        ...
        if (pos + length > size) {
            throw std::runtime_error("BencodeDecoder: String length exceeds data size");
        }
    ...
    BencodeValue decode_integer(const uint8_t* data, size_t size, size_t& pos) {
        ...
        // Parse the integer
        std::string int_str(reinterpret_cast<const char*>(data + int_start), pos - int_start);
    ...
    BencodeValue decode_dict(const uint8_t* data, size_t size, size_t& pos) {
        ...
        while (pos < size && data[pos] != 'e') {
            ...
            if (!std::isdigit(data[pos])) {
                throw std::runtime_error("BencodeDecoder: Dictionary key must be a string");
            }
